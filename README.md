...
Each developer maintains their own `config.local.json` that points to the data files they have available locally. This file is not committed — it is generated by the `just add-assembly-local` command and loaded into the JBrowse app at dev time.
...
Example:
```sh
just add-assembly-local test_data/canonical_core.fa
```
